...
def _receipt_key(site_id: int, key: str) -> str:
    raw = f"GR-IDEMP:{site_id}:{key.strip()}"
...
def create_goods_receipt(
    ...
    try:
        ...
        rkey = _receipt_key(site_id=int(po.site_id), key=idempotency_key)

        existing = db.execute(
            select(GoodsReceipt).where(GoodsReceipt.idempotency_key == rkey)
        ).scalar_one_or_none()
        if existing:
            return {
                "id": existing.id,
        ...
        return {
            ...
            "po_id": po.id,
            "to_location_id": payload.to_location_id,
            "idempotency_key": gr.idempotency_key,
        }
