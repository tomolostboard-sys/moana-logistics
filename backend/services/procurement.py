...
# PO considérés comme "engagés" dans le calcul du on_order
ENGAGED_PO_STATUSES = {
    POStatus.approved,
    POStatus.shipped,
    POStatus.partial,
...
def get_inbound_dock_location_id(db: Session, site_id: int) -> int:
    ...
    Priorité stricte à 'TAH-DOCK', sinon première DOCK trouvée.
    """
    loc = (
        db.execute(
            select(Location)
...
def rebuild_qty_on_order(
    ...
    *,
    ...
    - idempotent
    - transaction-safe
    - verrouille les lignes StockLevel (FOR UPDATE)
    """
    ...
    for pid in product_ids:
        ...
        stock_level.qty_on_order = outstanding
